...
class Simple_Page_Ordering {
	...
	public static function wp() {
		if ( 0 === strpos( get_query_var('orderby'), 'menu_order' ) ) {
			...
			wp_enqueue_script( 'simple-page-ordering', plugins_url( 'js/'.$script_name, __FILE__ ), array('jquery-ui-sortable'), '2.1', true );
			wp_enqueue_style( 'simple-page-ordering', plugins_url( 'css/simple-page-ordering.css', __FILE__ ));
		}
	}
	...
	public static function sort_by_order_link( $views ) {
		...
		$views['byorder'] = sprintf('<a href="%s" class="%s">%s</a>', $query_string, $class, __("Sorbarendezés", 'simple-page-ordering'));

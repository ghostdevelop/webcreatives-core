...
jQuery.QapTcha = {
	build : function(options)
	{
        var defaults = {
			...
			disabledSubmit : true,
			autoRevert : true,
			PHPfile : ajax_urls.admin_url+'/process.php',
			autoSubmit : false
        };
		if(this.length>0)
		return jQuery(this).each(function(i) {
			...
			if(opts.disabledSubmit) form.find('input[type=\'submit\']').hide();
			if(opts.disabledSubmit) form.find('input[type=\'button\']').attr("disabled", "disabled");
			if(opts.disabledSubmit) form.find('input[type=\'button\']').hide();
